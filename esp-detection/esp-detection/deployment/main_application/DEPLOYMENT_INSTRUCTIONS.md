...
```bash
cd deployment/grape_leaf_detect_camera/esp-dl/examples/grape_leaf_detect
```

### Step 2: Clean Previous Build (CRITICAL!)
```bash
...
**⚠️ IMPORTANT:** This clean step is CRITICAL because of the model embedding fix. Skipping it may cause the ESP32 to crash!
...
**✅ VERIFY Model Embedding:** Check the build output for these messages:
```
-- 🔧 DIRECT EMBED: Found model at ...
-- 🔧 DIRECT EMBED: Will embed .../grape_leaf_detect.espdl as RODATA
[xxx/xxx] 🔧 DIRECT EMBED: Copying model to build directory...
```

**❌ If you DON'T see these messages, STOP!** The model won't be embedded and your ESP32 will crash. Contact support.

Also check firmware size increased:
```
esp_image: segment 3: ... size=XXXXXX
```
Should be ~1,750,000+ bytes (not 1,277,720)
